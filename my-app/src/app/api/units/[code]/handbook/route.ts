...
export async function GET(_request, { params }) {
  ...
  if (!buildId)
    return NextResponse.json(
      { error: "Could not fetch handbook build ID" },
      { status: 502 },
    );
  ...
  try {
    ...
    if (json.message || !json.pageProps?.pageContent) {
      return NextResponse.json(
        { error: "Unit not found in handbook" },
        { status: 404 },
      );
    }
    ...
    const offerings: { period: string; location: string; mode: string }[] = (
      pc.unit_offering ?? []
    )
    ...
    const assessments: { name: string; weight: string; type: string }[] = (
      pc.assessments ?? []
    ).map((a: any) => ({
    ...
    const requisites: {
      type: string;
      groups: { connector: string; codes: string[] }[];
    }[] = (pc.requisites ?? [])
      ...
      .filter(
        (r: any, i: number, arr: any[]) =>
          arr.findIndex((x: any) => x.type === r.type) === i,
      );
    ...
    return NextResponse.json({
      ...
      workload: pc.workload_requirements
        ? stripHtml(pc.workload_requirements)
        : "",
      offerings,
      assessments,
      requisites,
