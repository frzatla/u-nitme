...
import { Plan, Profile, Schedule, ScheduledUnit, UnitCategory } from "@/lib/types";
...
const INTEREST_KEYWORDS: Record<string, string> = {
  ...
  "Problem Solving":"algorithms data structures problem solving computational mathematics",
  "Cybersecurity":  "cybersecurity security network forensics vulnerability malware",
  ...
  "Pitching":       "entrepreneurship startup venture pitching communication presentation",
  ...
  "Sustainability": "sustainability environmental climate ecology renewable energy",
...
function prereqsSatisfied(
  ...
  for (const rule of prereqList) {
    ...
    for (const orGroup of andGroups) {
      if (!orGroup) continue;
      const alternatives = orGroup.split(";").map((c) => c.trim()).filter(Boolean);
      if (alternatives.length && !alternatives.some((c) => completed.has(c))) return false;
    }
  }
  return true;
}
...
async function recommendElectives(
  ...
  try {
    ...
    const unitsDb: Record<string, any> = JSON.parse(readFileSync(MOCK_UNITS_PATH, "utf-8"));
    ...
    const updatedSemesters = schedule.schedule.map((s) => ({ ...s, units: [...s.units] }));
    ...
    for (let semIdx = 0; semIdx < updatedSemesters.length; semIdx++) {
      ...
      const maxLevel = sem.semester_index <= 1 ? 1 : sem.semester_index <= 3 ? 2 : 3;
    ...
    return { schedule: { ...schedule, schedule: updatedSemesters }, electiveCodes };
  } catch (err) {
...
function collapseOverflow(schedule: Schedule, maxSemesters: number): Schedule {
  ...
  const overflowSems = sems.filter((s) => s.period !== null && s.semester_index > maxSemesters);
  if (overflowSems.length === 0) return schedule;

  const overflowUnits = overflowSems.flatMap((s) => s.units.filter((u) => u.code !== "ELECTIVE"));
  ...
  const filteredSems = sems.filter((s) => s.period === null || s.semester_index <= maxSemesters);
...
export default async function NewPlanPage() {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    const maxSemesters = (Number(formData.get("yearEnd")) - Number(formData.get("yearStart")) + 1) * 2;
    ...
    if (rawSchedule && interests.length > 0) {
      const planCodes = new Set<string>(
        rawSchedule.schedule.flatMap((s) => s.units.map((u) => u.code)).filter((c) => c !== "ELECTIVE"),
      );
      const result = await recommendElectives(interests, rawSchedule, planCodes, maxSemesters);
      rawSchedule   = result.schedule;
      electiveCodes = result.electiveCodes;
    }

    if (rawSchedule) {
      newPlan.schedule = enrichCategories(rawSchedule, courseCode, aosCode, minorMajorType || undefined, minorMajorCode || undefined, electiveCodes);
    }
