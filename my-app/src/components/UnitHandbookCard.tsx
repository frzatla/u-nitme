      ...
      {!loading && data && (
        <>
          ...
          {uniqueOfferings.length > 0 && (
            <div className="px-5 py-4">
              <p
                className="mb-2.5 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.1em]"
                style={{ color: "#0EA5E9" }}
              >
              ...
              <ul className="space-y-1.5">
                {data.assessments.map((a, i) => (
                  <li key={i} className="flex items-center justify-between gap-3">
                    <span className="min-w-0 truncate text-[13px] text-black/60">
                      {a.name}
                    </span>
                    {a.weight && (
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                      {req.groups.map((g, gi) => (
                        <span key={gi} className="flex items-center gap-1.5">
                          {gi > 0 && (
                            <span className="text-[11px] font-semibold uppercase text-black/25">
                              and
                            </span>
                          )}
