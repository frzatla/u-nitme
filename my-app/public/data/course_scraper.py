"""
Monash Handbook - Course Scraper v1
=====================================
Discovers ALL courses from ALL faculties via the Monash Handbook sitemap,
scrapes each course page, and saves all results to final_courses.json.

Output structure matches c.json:
...
Usage:
    python course_scraper.py                        # scrapes current year
    python course_scraper.py --year 2025
    python course_scraper.py --year 2025 --output my_courses.json
"""
...
# ──────────────────── SITEMAP DISCOVERY ───────────────────────
def discover_course_codes(year: str) -> list[str]:
    """Walk the sitemap index and return all course codes for `year`."""
    print(f"[1/3] Discovering all courses for {year} ...")
    try:
    ...
    for sm_url in sitemap_urls:
        ...
        for url in re.findall(r"<loc>(.*?)</loc>", resp.text):
            if f"/{year}/courses/" not in url:
                continue
            code = url.split("/courses/")[-1].upper().strip("/")
            if code and code not in seen:
                seen.add(code)
                codes.append(code)

    codes.sort()
    print(f"  Found {len(codes)} course codes total.")
    return codes
...
# ──────────────────── COURSE SCRAPER ──────────────────────────
def scrape_course(code: str, year: str, idx: int, total: int) -> tuple[str, dict] | None:
    print(f"  [{idx:>5}/{total}] {code} ...", end=" ", flush=True)

    pc = fetch_page_content(f"{BASE_URL}/{year}/courses/{code}")
    if not pc:
        print("FAILED")
        return None
...
def save_json(data: dict, filename: str):
    ...
    print(f"\nSaved {len(data)} course records to {filename}")


# ────────────────────────── CLI ───────────────────────────────
def main():
    parser = argparse.ArgumentParser(
        description="Scrape all Monash Handbook courses across all faculties"
    )
    parser.add_argument("--year",   "-y", default=DEFAULT_YEAR,
                        help=f"Handbook year (default: {DEFAULT_YEAR})")
    parser.add_argument("--output", "-o", default="final_courses.json",
                        help="Output JSON filename (default: final_courses.json)")
    args = parser.parse_args()

    print(f"\n{'='*60}")
    print(f"  Monash Course Scraper v1  |  Year: {args.year}")
    print(f"{'='*60}\n")

    codes = discover_course_codes(args.year)
    if not codes:
        print("No course codes found — check year or network connection.")
        return

    total = len(codes)
    print(f"\n[2/3] Scraping {total} course pages ...")

    all_courses = {}
    for i, code in enumerate(codes, 1):
        result = scrape_course(code, args.year, i, total)
        if result:
            course_code, course_data = result
            all_courses[course_code] = course_data

    print(f"\n[3/3] Saving results ...")
    save_json(all_courses, args.output)
